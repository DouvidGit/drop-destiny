...
| 2 | Bass Forge | 选择 Bass 核心性格，再调节 4 个宏观旋钮 | 生成用户自己的 Bass 预设，影响 Aggression / Movement / Harmony |
...
选择核心性格后显示 4 个宏观控制。预设只决定默认值，用户必须至少亲手调整其中一个：
...
宏观旋钮应平滑更新参数，避免 zipper noise；界面显示创作型名称，专业参数放在帮助提示中。
...
`choices`、宏观参数和演奏事件是事实来源；`dna` 与 `result` 都是可重新计算的派生数据。禁止在用户每次点击时永久累加 DNA，否则返回上一步换选项会重复加分。
...
const STATE = {
  ...
  // Bass Forge 的少量创作控制；0–100，预设会提供默认值，用户可继续调整
  bassMacros: {
    body: 50,
    growl: 50,
    wobble: 50,
  ...
  // 以下均由当前 choices / macros / performance 重新计算
  dna: { ...INITIAL_DNA },
  result: {
    primaryStyle: null,
    secondaryStyle: null,
...
用户试听/选择/演奏
        ↓
只更新 choices / bassMacros / groove / performance
        ↓
...
- [ ] 5 阶段流程可前进、返回和重新开始。
- [ ] 选择、Bass 宏观参数和演奏 Pattern 使用统一状态管理。
...
- [ ] Bass Forge 至少提供 Body、Growl、Wobble、Space 四个宏观控制。
...
<body>
  <div id="app">
    ...
    <main>
      ...
      <section id="bassForge">         # 阶段 2：Bass 预设 + 4 个宏观控制
