...
伴奏时长约 20–28 秒，结构为 2 bars Intro + 3 bars Build-up + 1 bar Pre-drop + 4 bars Drop A + 3 bars Drop B + 1 bar Final（14 bars 总计）。用户主 Bass 从第 7 小节起覆盖 Drop A / B，并在 Final 小节前结束。
...
| F4 | 选择或调整任一创作参数后，内部 DNA 重新计算，正在播放的音乐同步变化 | 运行回归测试并听声音变化 |
