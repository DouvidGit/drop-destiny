...
const outputRoot = path.join(root, 'exports', 'collider-web-stems');
const mp3Root = path.join(outputRoot, 'mp3');
const embeddedOutput = path.join(root, 'ending-assets.js');
...
function writeEmbeddedAssets(sourceRoot) {
  fs.mkdirSync(mp3Root, { recursive: true });
  const embedded = {};
  for (const item of endings) {
    const wav = path.join(sourceRoot, item.file);
    if (!fs.existsSync(wav)) throw new Error(`Missing web backing WAV: ${wav}`);
    const mp3 = path.join(mp3Root, item.file.replace(/\.wav$/i, '.mp3'));
    ffmpeg([
      '-y', '-i', wav,
    ...
    embedded[item.id] = {
      mime: 'audio/mpeg',
      base64: fs.readFileSync(mp3).toString('base64')
    };
  }
  const source = [
    '// Generated by dev/build-ending-assets.js. Do not edit by hand.',
    '(function (global) {',
    "  'use strict';",
    `  global.DropDestinyEndingAssets = ${JSON.stringify(embedded)};`,
    "})(typeof window !== 'undefined' ? window : global);",
    ''
  ].join('\n');
  fs.writeFileSync(embeddedOutput, source);
  console.log(`Embedded ${endings.length} high-bitrate ending stems in ${embeddedOutput}`);
}
...
function main() {
  ...
  if (process.argv.includes('--embed-existing')) {
    writeEmbeddedAssets(path.join(root, 'assets', 'endings'));
    return;
  }
  endings.forEach(render);
  writeEmbeddedAssets(outputRoot);
